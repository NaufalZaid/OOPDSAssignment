...
public:
    ...
    Ship* createShip(const string& type, char symbol, const string& team) {
        if (type == "Battleship") return new Battleship(symbol, team);
        if (type == "Cruiser") return new Cruiser(symbol, team);
        if (type == "Destroyer") return new Destroyer(symbol, team);
        if (type == "Frigate") return new Frigate(symbol, team);
